...
void ValidateFpuOperands(XEDPARSE* Parse, Inst* Instruction)
{
    ...
    for(int i = 0; i < type->InstructionCount; i++)
    {
        ...
        const char* iformStr = xed_iform_enum_t2str((const xed_iform_enum_t)(inst->_iform_enum));
...
void LookupTableInit()
{
    ...
    for(int i = 0; i < XED_MAX_INST_TABLE_NODES; i++)
    {
        ...
        if(type->InstructionCount >= (sizeof(type->Instructions) / sizeof(*type->Instructions)))//32->dynamic size, modify by ppshuai
